...
**v3.16.5** — 內部重構，畫面與資料沒有變化：最長的方法 `renderVals()`（579 行）拆成
15 個分段，新增畫面不再讓它繼續長。新增 `tests/e2e-render-vals.js`，把 213 個模板繫結
的鍵與型別存成快照比對，擋住「某個繫結悄悄消失」這種不會拋錯的退化。

**v3.16.4** — 同步狀態列不再顯示「待上傳照片 N 張」，只留「待同步 N 筆」。照片排隊時
紀錄早就在伺服器上了，那個數字回答不了這條列存在的問題。照片照常上傳，同步進行中仍會
顯示「上傳照片 3/5…」的進度。

**v3.16.3** — 按了「增加聯絡人」之後有退路了：多一顆「取消，返回上一位」（頁首的
「返回」在有聯絡人停放時做同一件事），把剛才那次新增收回來，第一位連同名片照片回到表單，
不會再留下一筆沒有客戶需求與公司背景的紀錄。資料紀錄的「操作」欄加寬到 156px——
`📷 ×4` 的張數標籤原本會把整張表格撐出一條水平捲軸。

**v3.16.2** — 資料紀錄與編輯摘要列把已選的選項顯示成中英並陳（`Charlene 蘇秋菊`），
匯出仍是純英文；完成頁的「完成，返回」取消淡化處理，位置與文字不變。
...
- **雲端同步需要自備伺服器。** v3.13 起客戶端可同步紀錄、v3.15 起連活動與欄位定義一起同步，但**正式伺服器尚不存在**——`server/` 下的是參考實作，用於測試與交付規格。未設定伺服器時 app 完全離線運作。**部署一台正式伺服器是目前唯一擋住「下一場展真的用同步」的東西**（[#9](https://github.com/art20217/exhibition_form/issues/9)）。
...
- **接管要等對方拉取才會生效，而拉取有前提。** 被接管的那台只有在**拉取**時才會知道自己失去了擁有權，而拉取只發生在活動列表或活動內頁、且手上沒有開啟中的紀錄（`pullAllowed()`）。觸發點四個：App 啟動、切回前景、網路恢復、按「立即同步」。實測（v3.16 第一次量，單一組平板、單一個網路）**都在一秒內**：按「立即同步」約 0.5 秒，切到別的 App 再切回來約 0.1 秒。**停在填單畫面時等多久都不會變**——那是刻意的，遠端資料不該蓋掉正在打字的內容，回到活動列表就會更新。
