...
平板端在「活動管理 → 系統設定」填入同步伺服器網址（含 `/v1`）與其中一組權杖。
...
活動列表右上角齒輪（PIN）→ 系統設定 → 同步伺服器填 `https://<隨機>.trycloudflare.com/v1`
→ 填該台的權杖 → **測試連線** → 儲存設定。
...
- **另一台平板要先同步到那場活動，才看得到紀錄**（v3.15 起活動會同步）。順序是：
  A 同步 → B 回活動列表按「立即同步」→ B 的列表出現那場活動 → 點進去才看得到 A 的紀錄。
  B 若停在自己的活動裡，畫面上什麼都不會變——每個讀取端都只比對目前開啟的活動。
- **欄位定義只有擁有者能改。** 建立活動的那台是預設擁有者，其他平板的欄位頁籤是唯讀的，
  要改得先在活動管理裡「接管欄位設定」。收單與匯出不受影響。
...
- `POST /v1/records` 與 `POST /v1/events` **冪等**，依 `id` upsert。同一批送兩次的結果要和送一次相同。
- `seq` 單調遞增，**重啟不得倒退**，而且不是時間戳。
- 墓碑（`deletedAt` 非 null）**必須保留並回傳**，否則已刪的紀錄會從沒同步的平板上長回來。
- 逐筆回報結果，**一筆壞資料不得讓整批失敗**。
- **活動與紀錄各有各的 `seq`**，兩個序號空間互不干擾。
- 照片以**照片 id** 定位（`PUT /v1/photos/<photoId>`），不是紀錄 id——一筆紀錄可以有多張，
  而用陣列位置定位會在刪掉中間一張時整排位移。
- 紀錄推上來時帶 `photoIds`，**不在裡面的照片要刪掉**。判斷時讀伺服器上既有的那一份，
  因為墓碑根本不帶 `photoIds`。
- 收到**紀錄**墓碑要刪掉該筆的所有照片；收到**活動**墓碑要刪掉該活動底下所有紀錄的照片。
- 非擁有者送來的 `fieldDefs` **拒收，但活動本身照收**（回 `fieldDefsRejected: true`）——
  同一筆可能還帶著任何平板都能做的狀態變更。
